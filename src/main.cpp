...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pixicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pixicoin Signed Message:\n";
...
{
    // Pixicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 1 * COIN; // A single Pixi per block

    // Subsidy stays constant
    // nSubsidy >>= (nHeight / 840000);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan =  60 * 60; // Pixicoin: every hour, we want to stay on track
static const int64 nTargetSpacing = 24 * 60 * 60 ; // Pixicoin: a single block per day
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pixicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pixicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Pixicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Pixicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PixicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PixicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PixicoinMiner(CWallet *pwallet)
{
    printf("PixicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pixicoin-miner");
    ...
    try { loop {
        ...
        printf("Running PixicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PixicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PixicoinMiner, pwallet));
}
